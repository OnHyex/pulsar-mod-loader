...
namespace PulsarModLoader.Utilities.ExceptionImprovements
{
    static class ExceptionTools
    {
        static T GetDelegate<T>(Type type, string name) where T : Delegate
        {
            ...
            {
                UnityEngine.Debug.LogError($"Harmony: Failed to create delegate for {type}.{name}: {ex.Message}");
                return null;
            }
        }
        ...
        static AccessTools.FieldRef<TInst, TField> SafeFieldRefAccess<TInst, TField>(string fieldName)
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"Harmony: Unable to access field {typeof(TInst).FullName}.{fieldName}: {ex.Message}");
                return null;
            }
        }
        ...
        internal static string ExtractHarmonyEnhancedStackTrace(StackTrace trace, bool forceRefresh, out int hash)
        {
            ...
            //Remove Stack Trace caching here
            //return stacktrace;
            ...
            {
                UnityEngine.Debug.LogWarning("Harmony: Clearing stacktrace cache to preserve memory");
                seenStacktraces.Clear();
            }

            var count = seenStacktraces.AddOrUpdate(hash, 1, (k, v) => v + 1);
            if (count > 3) return $"{hashRef} Duplicate stacktrace, see ref for original";
            return $"{hashRef}\n{stacktrace}";
        }
        ...
        static bool AddHarmonyFrames(this StringBuilder sb, StackTrace trace)
        {
            ...
            for (var i = 0; i < trace.FrameCount; i++)
            {
                try
                {
                    ...
                    {
                        method = (MethodInfo)Harmony.GetOriginalMethod(methodinfo);
                    }
                    else 
                    {
                        method = methodbase;
