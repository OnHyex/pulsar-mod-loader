...
namespace PulsarModLoader.Content.Talents
{
    public class HelperMethods
    {
        ...
        public static IEnumerable<CodeInstruction> Override63Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            new List<CodeInstruction>()
            {
                new CodeInstruction(OpCodes.Call, Method(typeof(HelperMethods), nameof(Override63TranspilerPatch)))
            },
            PatchMode.REPLACE);
        }
    ...
    public class HideTalentsFromTabMenu
    {
        ...
        {
            bool result = true;
            if (PMLConfig.HideResearchableTalentsFromTab.Value && !PLServer.Instance.IsTalentUnlocked(inTalent))
